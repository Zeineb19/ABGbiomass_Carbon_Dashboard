...
def load_lidar():
    ...
    if df is not None:
        df = df[df["tile"].isin([1, 2])].reset_index(drop=True)  # tiles 1+2 only (~14k rows)
        df["height_range_m"] = (df["height_p95_m"] - df["height_p25_m"]).astype("float32")
        df["tile_label"]     = df["tile"].map({1:"Tile 1", 2:"Tile 2"})
    return df
...
if section == "🛰 Spectral Models":
    ...
    with map_tab:
        ...
        st.info("Showing test-set prediction points only (no full raster).")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if df_lid is None:
        st.error("❌ `merged_tree_level.csv` is missing or empty in `/data/`. "
                 "Go to your GitHub repo → data/ → Upload files → drag the real CSV → Commit.")
        st.stop()
    ...
    TILE_COLORS = {"Tile 1":"#2dd4bf","Tile 2":"#86efac"}
