...
def load_spectral_summary():
    return load_csv("spectral_models_summary_metrics.csv")

@st.cache_data
def load_embedding_summary():
    return load_csv("embedding_models_summary_metrics.csv")

@st.cache_data
def load_lidar():
    df = load_csv("merged_tree_level.csv")
    if df is not None:
...
def embedding_test(model):
    return load_csv(f"embedding_{model.lower()}_test_predictions.csv")

def embedding_full(model):
    return load_csv(f"embedding_{model.lower()}_full_predictions.csv")
    ...
    with spatial_tab:
        ...
        if map_col == "tile_label":
            fig_map = px.scatter(
                ...
                color="tile_label", color_discrete_map=TILE_COLORS, opacity=0.5,
