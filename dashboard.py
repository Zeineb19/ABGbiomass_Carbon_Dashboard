...
# Resolve data/ folder — works locally AND on Streamlit Cloud
def _find_data_dir():
    candidates = [
    ...
    for c in candidates:
        probe = os.path.join(c, "merged_tree_level.csv")
        if os.path.isfile(probe) and os.path.getsize(probe) > 100:
            return c
    # fallback: return the first existing directory even if probe file missing
    for c in candidates:
        if os.path.isdir(c):
            return c
    return candidates[0]
...
_LIDAR_KEEP   = [
    "tree_id","tile","tile_tree_id",
    "crown_x","crown_y",
    "point_count","height_max_m","height_mean_m","height_std_m",
...
_LIDAR_DTYPES = {c:"float32" for c in _LIDAR_KEEP
                 if c not in ("tree_id","tile","tile_tree_id","point_count","canopy_pts","crown_x","crown_y")}
_LIDAR_DTYPES.update({"tree_id":"int32","tile":"int8",
                       "tile_tree_id":"int32","point_count":"int32","canopy_pts":"int32"})
...
def load_lidar():
    ...
    if df is not None:
        # Keep only columns that actually exist in this file version
        df = df[[c for c in _LIDAR_KEEP if c in df.columns]]
...
""")
    st.markdown("---")
    st.caption(f"📂 Data path: `{DATA_DIR}`")
...
if section == "🛰 Spectral Models":
    ...
    with map_tab:
        st.markdown("#### Spatial Map — Test-Set Points")
        ...
        df_pts = spectral_test(map_model, map_variant)
        if df_pts is None:
            st.warning(f"⚠ Test file not found for {map_model} {map_variant}.")
        else:
            c1, c2 = st.columns(2)
            with c1: st.plotly_chart(spatial_map(df_pts, "y_true", "Observed (test set)"),
                                     use_container_width=True)
            with c2: st.plotly_chart(spatial_map(df_pts, "y_pred",
                                     f"Predicted — {map_model} ({map_variant.replace('_',' ')})"),
                                     use_container_width=True)
            df_pts = df_pts.copy()
            df_pts["diff"] = df_pts["y_pred"] - df_pts["y_true"]
            fig_diff = go.Figure(go.Scattergl(
                x=df_pts["lon"], y=df_pts["lat"], mode="markers",
                marker=dict(size=5, color=df_pts["diff"], colorscale="RdBu", cmid=0,
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with map_tab:
        ...
        df_pts = embedding_test(map_model)
        if df_pts is None:
            st.warning(f"⚠ Test file not found for {map_model}.")
        else:
            c1, c2 = st.columns(2)
            with c1: st.plotly_chart(spatial_map(df_pts, "y_true", "Observed GEDI MU (test set)"),
                                     use_container_width=True)
            with c2: st.plotly_chart(spatial_map(df_pts, "y_pred",
                                     f"Predicted — {map_model} (Embeddings)"),
                                     use_container_width=True)
            df_pts = df_pts.copy()
            df_pts["diff"] = df_pts["y_pred"] - df_pts["y_true"]
            fig_diff = go.Figure(go.Scattergl(
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown(
        "Tree-level structural attributes derived from **LiDAR point-cloud segmentation** "
        "over the Veluwe — 3 tiles, ~19 500 individual trees."
    )
    ...
    if df_lid is None:
        st.error(
            "❌ `merged_tree_level.csv` not found or empty in `/data/`. "
            "Go to your GitHub repo → data/ → Upload files → drag the CSV → Commit."
        )
        st.stop()

    LIDAR_FEATS = [
        "height_max_m","height_mean_m","height_std_m","height_range_m",
        "height_p25_m","height_p50_m","height_p75_m","height_p90_m","height_p95_m",
        "cv_height",
    ...
    FEAT_LABELS = {
        ...
        "cv_height":      "Coefficient of Variation Height",
        "crown_radius_m": "Crown Radius (m)",
        "crown_r95_m":    "Crown Radius P95 (m)",
    ...
    with overview_tab:
        ...
        c4.metric("Mean Point Density",    f"{df_lid['density_pt_m2'].mean():.1f} pt/m²")
        c5.metric("Tiles",                 f"{df_lid['tile'].nunique()}")

        st.markdown("---")
        tile_counts = df_lid["tile_label"].value_counts().sort_index().reset_index()
        ...
        with st.expander("📋 Per-tile Summary Statistics"):
            st.dataframe(
                df_lid.groupby("tile_label")[
                    ["height_max_m","height_mean_m","crown_area_m2",
                     "density_pt_m2","point_count","cv_height"]
                ].describe().round(2),
                use_container_width=True
            )
    ...
    with dist_tab:
        ...
        with c1:
            feat_sel = st.selectbox("Feature", LIDAR_FEATS,
        ...
        fig_vio = px.violin(df_lid, y=feat_sel, x="tile_label",
                            ...
                            labels={"tile_label":"Tile",
                                    feat_sel: FEAT_LABELS.get(feat_sel, feat_sel)})
    ...
    with struct_tab:
        ...
        corr_df = df_lid[LIDAR_FEATS].rename(columns=FEAT_LABELS).corr().round(3)
        fig_corr = px.imshow(corr_df, text_auto=".2f",
                             color_continuous_scale="RdBu_r", zmin=-1, zmax=1,
                             aspect="auto",
                             title="Pearson Correlation — LiDAR Structural Features")
        fig_corr.update_layout(**PL, height=620)
        ...
        with cc1:
            xf = st.selectbox("X axis", LIDAR_FEATS, index=0,
                              format_func=lambda x: FEAT_LABELS.get(x, x), key="lx")
        with cc2:
            yf = st.selectbox("Y axis", LIDAR_FEATS, index=12,
                              format_func=lambda x: FEAT_LABELS.get(x, x), key="ly")

        fig_sc = px.scatter(
            df_lid.sample(min(5000, len(df_lid)), random_state=42),
            x=xf, y=yf, color="tile_label",
            opacity=0.5, color_discrete_map=TILE_COLORS,
        ...
        st.markdown("#### 3D — Height × Crown Area × Point Density")
        sample = df_lid.sample(min(3000, len(df_lid)), random_state=42)
        fig3d = px.scatter_3d(
            sample, x="height_max_m", y="crown_area_m2", z="density_pt_m2",
            color="tile_label", opacity=0.6, color_discrete_map=TILE_COLORS,
            labels={"height_max_m":"Max Height (m)", "crown_area_m2":"Crown Area (m²)",
                    "density_pt_m2":"Point Density (pt/m²)", "tile_label":"Tile"},
            title="3D — Tree Height × Crown Area × Point Density",
        )
        ...
        fig3d.update_layout(**PL, height=540,
                            scene=dict(bgcolor="rgba(11,17,23,0.8)",
                                       ...
                                       zaxis_title="Point Density (pt/m²)"))
        st.plotly_chart(fig3d, use_container_width=True)

    # ── SPATIAL MAP ────────────────────────────────────────────────────────────
    with spatial_tab:
        st.markdown("#### Spatial Distribution of Individual Trees")
        map_col = st.selectbox(
            "Colour trees by",
            ["tile_label"] + LIDAR_FEATS,
            format_func=lambda x: "Tile" if x == "tile_label" else FEAT_LABELS.get(x, x),
        )

        if map_col == "tile_label":
            fig_map = go.Figure()
            for tname, tcolor in TILE_COLORS.items():
                sub = df_lid[df_lid["tile_label"] == tname]
                fig_map.add_trace(go.Scattergl(
                    x=sub["tree_lon"], y=sub["tree_lat"], mode="markers",
                    marker=dict(size=3, color=tcolor, opacity=0.6),
                    name=tname,
                ))
            fig_map.update_layout(**PL, height=540, title="Individual Tree Locations by Tile",
                                  xaxis_title="Longitude", yaxis_title="Latitude")
        else:
            fig_map = go.Figure(go.Scattergl(
                x=df_lid["tree_lon"], y=df_lid["tree_lat"], mode="markers",
                marker=dict(size=3, color=df_lid[map_col],
